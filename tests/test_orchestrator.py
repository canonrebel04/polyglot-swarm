...
def test_coordinator_initialization():
    ...
    assert coordinator._task_queue.empty()
...
def test_coordinator_task_decomposition():
    ...
    tasks = asyncio.run(coordinator.decompose_task("Implement user authentication"))
...
def test_watchdog_initialization():
    ...
    assert watchdog._last_activity == {}
...
async def test_watchdog_monitoring():
    ...
    assert "test-agent-1" in watchdog._last_activity
    assert "test-agent-2" in watchdog._last_activity
...
async def test_global_instances():
    ...
    assert hasattr(coordinator, 'decompose_task')
